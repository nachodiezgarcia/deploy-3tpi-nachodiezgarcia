  ...
  const play = async () => {
    ...
    try {
      ...
      if (errorName === 'NotAllowedError') {
        console.error(
          'Video play was blocked. This might be due to autoplay policy or CORS issues.',
        );
      } else if (
        errorMessage.includes('CORS') ||
        errorMessage.includes('ORB') ||
        errorMessage.includes('blocked')
      ) {
        console.error(
          'CORS/ORB error detected. The video source might not allow cross-origin access.',
        );
      }
