...
export const createUIController = (
  ...
  let progressDragHandler: ReturnType<typeof createProgressDragHandler> | null =
    null;
  let speedControlHandler: ReturnType<typeof createSpeedHandler> | null = null;
  let fullscreenControlsHandler: ReturnType<
    typeof createFullscreenHandler
  > | null = null;
  ...
  const handleProgressBarClick = (e: Event) => {
    ...
    if (
      !elements.progressThumb.contains(clickedElement) &&
      clickedElement !== elements.progressThumb
    ) {
      const rect = elements.progressBar.getBoundingClientRect();
      const percentage = player.calculateSeekPercentage(
        mouseEvent.clientX,
        rect,
      );
  ...
  const handleVolumeTrackClick = (e: Event) => {
    ...
    if (
      !elements.volumeThumb.contains(clickedElement) &&
      clickedElement !== elements.volumeThumb
    ) {
  ...
  const setupHandlers = () => {
    ...
    fullscreenControlsHandler = createFullscreenHandler(
      player,
      container,
      elements.controls,
    );
  ...
  const destroy = () => {
    ...
    elements.fullscreenButton.removeEventListener(
      'click',
