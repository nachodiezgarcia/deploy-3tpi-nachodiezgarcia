...
export const createMobileHandler = (
  ...
  const { tapZones, controls, gestureOverlay, gestureIcon, gestureText } =
    elements;
  ...
  const onTapZone = (event: Event) =>
    player.onTapZone(tapZoneHandler)(event as TouchEvent);
